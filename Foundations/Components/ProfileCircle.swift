...
struct ProfileCircle: View {
    @Binding private var imageString: String;
    ...
    init(_ imageString: String = "",
        ...
        if isBase64 {
            self._imageString = bindingImage
        } else {
            self._imageString = State(
                wrappedValue: imageString
            ).projectedValue
        }
    }
    ...
    var body: some View {
        VStack {
            getImage()
                .resizable()
                .padding(.vertical, imageString == "" ? radius / 20 : 0)
